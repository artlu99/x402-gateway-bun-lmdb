...
async function handleHealth(): Promise<Response> {
	...
	const evmNetworks = networkKeys.filter(
		(k) => SUPPORTED_NETWORKS[k]?.vm === "evm",
	);
	...
	return corsJson({
		...
		routes: Object.keys(ROUTE_CONFIG)
			.map((key) => {
...
function handleDiscovery(req: Request): Response {
	...
	const resources = Object.values(ROUTE_CONFIG).map(
	...
	const chainNames = networkKeys.map((caip2) => {
		...
		return net.chainId !== undefined
			? (chainMap[net.chainId] ?? `Chain ${net.chainId}`)
			: "Unknown";
	});

	// Build route documentation
	const routeDocs = Object.entries(ROUTE_CONFIG)
		.map(([_key, route]) =>
			[
...
// ============================================================
function createPaidRouteHandler(
	routeKey: string,
...
// Log active networks
const networkKeys = Object.keys(SUPPORTED_NETWORKS);
const evmCount = networkKeys.filter(
	(k) => SUPPORTED_NETWORKS[k]?.vm === "evm",
).length;
const svmCount = networkKeys.filter(
	(k) => SUPPORTED_NETWORKS[k]?.vm === "svm",
).length;
console.log(
	`[x402-gateway] Active networks (${networkKeys.length}): ${evmCount} EVM, ${svmCount} SVM`,
);
...
networkKeys.forEach((caip2) => {
	...
	const chainLabel =
		net.chainId !== undefined ? `chain ${net.chainId}` : net.vm.toUpperCase();
