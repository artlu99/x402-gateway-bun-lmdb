...
describe("Route Configuration - Real Module Tests", () => {
	describe("ROUTE_CONFIG", () => {
		...
		test("should have backendApiKeyEnv for all routes", () => {
			for (const [key, route] of Object.entries(ROUTE_CONFIG)) {
				expect(
					route.backendApiKeyEnv,
					`Route ${key} missing backendApiKeyEnv`,
				).toBeDefined();
		...
		test("should have backendApiKeyHeader for all routes", () => {
			for (const [key, route] of Object.entries(ROUTE_CONFIG)) {
				expect(
					route.backendApiKeyHeader,
					`Route ${key} missing backendApiKeyHeader`,
				).toBeDefined();
			}
		});
	...
	describe("ALL_NETWORKS", () => {
		...
		test("should have Solana network", () => {
			expect(
				ALL_NETWORKS["solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp"],
			).toBeDefined();
			expect(ALL_NETWORKS["solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp"].vm).toBe(
				"svm",
			);
		});
		...
		test("all networks should have token with required fields", () => {
			for (const [caip2, network] of Object.entries(ALL_NETWORKS)) {
				expect(
					...
					`${caip2} missing token.address`,
				).toBeDefined();
				expect(
					network.token.decimals,
					`${caip2} missing token.decimals`,
				).toBeDefined();
			}
		});
	...
	describe("Facilitator configuration", () => {
		...
		test("facilitator contract should be valid address", () => {
			const megaeth = ALL_NETWORKS["eip155:4326"];
			expect(megaeth.facilitator.facilitatorContract).toMatch(
				/^0x[a-fA-F0-9]{40}$/,
			);
		});
	});
	...
	describe("SUPPORTED_NETWORKS (active networks)", () => {
		...
		test("should only include networks with RPC URLs configured", () => {
			...
			for (const caip2 of networkKeys) {
				...
				if (network.vm === "svm") {
					...
					expect(
						rpcUrl,
						`${caip2} should have RPC URL if in SUPPORTED_NETWORKS`,
					).toBeDefined();
				}
			}
		});
		...
		test("SVM networks should require SOLANA_FACILITATOR_PRIVATE_KEY", () => {
			const networkKeys = Object.keys(SUPPORTED_NETWORKS);
			const svmNetworks = networkKeys.filter(
				(k) => SUPPORTED_NETWORKS[k].vm === "svm",
			);
	...
	describe("USDC contract addresses", () => {
		test("Base USDC should have correct address", () => {
			expect(ALL_NETWORKS["eip155:8453"].token.address.toLowerCase()).toBe(
				"0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913".toLowerCase(),
			);
		});

		test("Ethereum USDC should have correct address", () => {
			expect(ALL_NETWORKS["eip155:1"].token.address.toLowerCase()).toBe(
				"0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48".toLowerCase(),
			);
		});

		test("Arbitrum USDC should have correct address", () => {
			expect(ALL_NETWORKS["eip155:42161"].token.address.toLowerCase()).toBe(
		...
		test("Solana USDC should have correct mint address", () => {
			expect(
				ALL_NETWORKS["solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp"].token.address,
		...
		test("all EVM addresses should be valid 40-char hex", () => {
			for (const [caip2, network] of Object.entries(ALL_NETWORKS)) {
				if (network.vm === "evm") {
					expect(network.token.address, `${caip2} address format`).toMatch(
						/^0x[a-fA-F0-9]{40}$/,
					);
				}
			}
	...
	describe("RPC environment variables", () => {
		test("should have correct env var names", () => {
			...
			expect(
				ALL_NETWORKS["solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp"].rpcEnvVar,
			).toBe("SOLANA_RPC_URL");
		});
	});
		...
		test("chain IDs should match CAIP-2", () => {
			for (const [caip2, network] of Object.entries(ALL_NETWORKS)) {
				if (network.vm === "evm") {
					...
					expect(network.chainId, `${caip2} chainId mismatch`).toBe(
						chainIdFromCaip2,
					);
				}
			}
