...
describe('Comprehensive Unit Tests', () => {
  ...
  describe('URL construction', () => {
    ...
    test('should construct correct URLs', () => {
      expect(buildBackendUrl('https://api.example.com', '/api/endpoint'))
        .toBe('https://api.example.com/api/endpoint');
    });
    ...
    test('should handle nested paths', () => {
      expect(buildBackendUrl('https://api.example.com', '/v1/users/123/profile'))
        .toBe('https://api.example.com/v1/users/123/profile');
    });
  });

  describe('Request header handling', () => {
    function getPaymentHeader(headers) {
      // Case-insensitive header lookup
      const keys = Object.keys(headers);
      const paymentKey = keys.find(k => k.toLowerCase() === 'payment-signature');
    ...
    test('should return null when no payment header', () => {
      ...
      expect(getPaymentHeader({ 'Content-Type': 'application/json' })).toBeNull();
    });
  });

  describe('Response header generation', () => {
    function createPaymentResponseHeader(data) {
  ...
  describe('Facilitator request building', () => {
    function buildFacilitatorRequest(paymentPayload, routeConfig, network) {
      ...
      const facilitatorNetwork = network.facilitator.networkName || paymentPayload.network;
      const facilitatorPayTo = network.facilitator.facilitatorContract || routeConfig.payTo;
    ...
    test('should build facilitator request with correct structure', () => {
      ...
      const request = buildFacilitatorRequest(paymentPayload, routeConfig, network);
      ...
      expect(request.paymentRequirements.maxAmountRequired).toBe('10000000000000000');
    });
  });
  ...
  describe('Subpath handling', () => {
    ...
    test('should join array params', () => {
      expect(getSubpath({ path: ['users', '123', 'profile'] })).toBe('users/123/profile');
    });
  ...
  describe('Path alias resolution', () => {
    function resolvePath(subpath, aliases) {
  ...
  describe('Agent discovery response', () => {
    test('should include all route information', () => {
      ...
      expect(accepted.routes[0].networks).toHaveLength(1);
    });
  });
