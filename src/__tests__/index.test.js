...
describe('Endpoint Response Structures', () => {
  ...
  describe('Accepted endpoint response', () => {
    test('should have correct structure', () => {
      ...
      expect(acceptedResponse.routes[0].extensions['payment-identifier'].supported).toBe(true);
      expect(acceptedResponse.routes[0].extensions.bazaar.discoverable).toBe(true);
    });
  });
  ...
  describe('402 Payment Required response', () => {
    ...
    test('should include PAYMENT-REQUIRED header', () => {
      const headerPayload = {
        x402Version: 2,
        accepts: [],
        resource: { url: 'https://test.com/v1/test', description: 'Test', mimeType: 'application/json' },
        extensions: {},
      };
...
describe('Payment Payload Parsing', () => {
  test('should parse valid payment header', () => {
    ...
    const headerBase64 = Buffer.from(JSON.stringify(payload)).toString('base64');
    const decoded = JSON.parse(Buffer.from(headerBase64, 'base64').toString());
  ...
  test('should handle X-Payment header alias', () => {
    ...
    const paymentHeader2 = headers2['payment-signature'] || headers2['x-payment'];
...
describe('Payment Identifier Extension', () => {
  ...
  test('should reject identifier with invalid characters', () => {
    const payload = {
      extensions: {
        'payment-identifier': { paymentId: 'test-payment-id-with-special-chars!@#$' },
      },
    };
    expect(extractPaymentIdentifier(payload)).toBeNull();
  });
  ...
  test('should accept underscore and hyphen', () => {
    const payload = {
      extensions: {
        'payment-identifier': { paymentId: 'test_id-with_underscores-and-hyphens' },
      },
    };
    expect(extractPaymentIdentifier(payload)).toBe('test_id-with_underscores-and-hyphens');
  });
});

describe('Decimal Price Scaling', () => {
  function scalePrice(basePriceAtomic, baseDecimals, tokenDecimals) {
